...
<x-datatable :dataTable="$dataTable" :title="__('messages.points')">
    <x-slot:header>
{{--        @can('create muscles')--}}
            <div class="card-header pb-0">
                <div class="row row-xs wd-xl-80p">
                    <div class="col-sm-6 col-md-3 mg-t-10">
                        <a href="{{route('points.create')}}" class="btn btn-info-gradient btn-block"
    ...
    <x-slot:script>
        <script>
            ...
            function updateMuscleStatus(id) {
                ...
                $.ajax({
                    url: '{{ route('update-points-status', ":id") }}'.replace(':id', id),
                    type: 'GET', // Use HTTP GET method here
                    success: function(response) {
                        $('#points-table').DataTable().ajax.reload();
                    },
