        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-body">
                    ...
                    <form id="fileUploadForm" class="needs-validation was-validated" action="{{route('points.update',$points->id)}}" method="post" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="row row-sm">
                            <div class="form-group col-md-6 has-success mg-t-10">
                                <label for="title_ar">{{ __('messages.name') }}  :</label>
                                <input id="name" type="text" class="form-control " name="name" value="{{$points->name}}"  required>
                                @error('name')
                            ...
                            <div class="form-group col-md-6 has-success mg-t-10">
                                ...
                                <input id="points" type="number" class="form-control " name="point" value="{{$points->point}}"  required>
                                @error('points')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
